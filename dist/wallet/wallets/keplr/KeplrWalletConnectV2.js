...
export class KeplrWalletConnectV2 extends ConnectedWallet {
    ...
    async signAndBroadcastTx({ msgs, memo, timeoutHeight }, fee, accountNumber, sequence) {
        ...
        let txRaw;
        if (this.useAmino) {
            const { signed, signature } = await this.wc.signAmino(this.chainId, this.address, tx.toStdSignDoc(params));
            txRaw = tx.toSignedAmino(signed, signature.signature);
        }
        else {
            const { signed, signature } = await this.wc.signDirect(this.chainId, this.address, tx.toSignDoc(params));
